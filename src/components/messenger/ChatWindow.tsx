...
const ChatWindow = ({ chat, messages, onSendMessage, currentUser, userTyping }: ChatWindowProps) => {
  ...
  const getStatusText = () => {
    if (chat.isGroup) {
      const onlineCount = chat.participants.filter((p: any) => p.isOnline && p.id !== currentUser.id).length;
      return `${chat.participants.length} участников, ${onlineCount} в сети`;
    } else {
        ...
        <Dialog open={showChatInfo} onOpenChange={setShowChatInfo}>
          ...
          <DialogContent>
            ...
            <div className="space-y-4 mt-4">
              ...
              <div>
                <h4 className="font-medium mb-2">Участники ({chat.participants.length})</h4>
                  ...
                  {chat.participants.map((participant: any) => (
                    <div key={participant.id} className="flex items-center gap-2">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {participant.isOnline ? "В сети" : formatLastSeen(participant.lastSeen)}
                        </div>
                      </div>
        ...
        {messages.map((message) => {
          ...
          return (
            ...
            >
              ...
              >
                {!isMine && !chat.isGroup && (
                  <div className="text-xs font-medium mb-1">{sender?.displayName}</div>
                )}
        ...
        {userTyping && (
          <div className="flex justify-start">
            <div className="bg-white border rounded-lg p-3 max-w-[70%]">
              <div className="text-xs font-medium mb-1">{otherParticipant?.displayName}</div>
